...
export default class extends Controller {
  ...
  updateTriggerContent() {
    ...
    if (checkedInputs.length === 0) {
      this.triggerContentTarget.innerText =
  ...
  filterItems(e) {
    ...
    if (this.hasToggleAllTarget) {
      if (filterTerm)
        this.toggleAllTarget.parentElement.classList.add("hidden");
  ...
  focusSelectedInput() {
    const visibleInputs = this.inputTargets.filter(
  ...
  wrapSelectedInputIndex(length) {
    this.selectedItemIndex =
      ((this.selectedItemIndex % length) + length) % length;
  }
